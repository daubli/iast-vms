...
abstract class AbstractStringBuilder implements Appendable, CharSequence {
    ...
    public String substring(int start, int end) {
        checkRangeSIOOBE(start, end, count);
        if (isLatin1()) {
            String result = StringLatin1.newString(value, start, end - start);
            result.setTaint(getTaint());
            return result;
        }
        String result = StringUTF16.newString(value, start, end - start);
        result.setTaint(getTaint());
        return result;
    }
